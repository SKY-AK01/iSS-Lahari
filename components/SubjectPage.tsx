...
import { Map, FileText, Plus, Trash2, X, Pencil, Check } from 'lucide-react';
            ...
            <div key={chapter.id} style={{ position: 'relative' }}>
              ...
              {isMentor && editingChapterId === chapter.id ? (
                ...
                <div
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem 1.25rem', paddingRight: isMentor ? '5.5rem' : '1.25rem', border: 'var(--border-thick)', boxShadow: 'var(--shadow-btn)', background: 'var(--bg-3)', cursor: 'pointer', flexWrap: 'wrap', transition: 'all 100ms' }}
              ...
              {/* Edit + Delete buttons (shown only when not in rename mode) */}
              {isMentor && editingChapterId !== chapter.id && (
